...
void initGame(void){
    ...
    background_image = al_load_bitmap("Assets/bitart_background.png");
    if (!background_image) game_abort("failed to load background image");
...
void start_loop(void){
    ...
    while(!gameDone){ // Will loop until the program exitd
        ...
        if(redraw && al_is_event_queue_empty(event_queue))
        {
            ...
            // Draw Background
            float bg_width = al_get_bitmap_width(background_image);
            float bg_height = al_get_bitmap_height(background_image);
            float scale_x = (float)SCREEN_W / bg_width;
            float scale_y = (float)SCREEN_H / bg_height;
            al_draw_scaled_bitmap(background_image, 0, 0, bg_width, bg_height, 0, 0, SCREEN_W, SCREEN_H, 0);
