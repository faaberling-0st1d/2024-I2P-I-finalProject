...
bool update_bullet(Bullet * bullet, enemyNode * enemyList, Map * map){
    ...
    */
    ...
    // Remember the difference of the coord sys!
    ...
    if ( tile_x < 0 || tile_x + 1 > map->row || tile_y < 0 || tile_y > map->col || map->map[tile_x][tile_y] == WALL || (map->map[tile_x][tile_y] == DOOR_CLOSE && door_anime_tick < (42 * 3 - 1)) ) {
        return true;
    }
    
    // Check if the bullet collide with the enemies by simple iterating
    // printf("Test: bulletCoord: (%lf, %lf)\n", bullet->coord.x, bullet->coord.x);
    ...
    while(cur != NULL){
        ...
        */
        ...
        if (((bullet->coord.x >= enemyCoord.x) && (bullet->coord.x <= enemyCoord.x + TILE_SIZE))\
                && ((bullet->coord.y >= enemyCoord.y) && (bullet->coord.y <= enemyCoord.y + TILE_SIZE)) ) {
