...
bool updateEnemy(Enemy * enemy, Map * map, Player * player){
    ...
    if (enemy->status == DYING) {
        /* [TODO Homework III 12.24] Death animation tick; Return true */
    ...
    enemy->animation_tick = (enemy->animation_tick + 1) % 64; // One frame takes "8"!
    ...
    if(enemy->knockback_CD > 0){
        ...
        /* [TODO Homework O 12.25] Enemy Moving Torwards Player */
...
void drawEnemy(Enemy * enemy, Point cam){
    ...
    if (enemy->status == ALIVE) {
        ...
        int offset = 16 * (int)(enemy->animation_tick / 8); // 8 means: each 8 ticks correspond to a frame
        
        // If being hit ==> use the right 8 frames in the first row.
        if (enemy->animation_hit_tick > 0) {
            offset += 16 * 8;
        }

        int flag = enemy->dir == RIGHT ? 1 : 0;            // Character flipping.
        int tint_red = enemy->knockback_CD > 0 ? 255 : 0;  // When hitted, tint_red = 255, revealing the original color of slime. 
                                                          
        if (enemy->type == slime) {
            al_draw_tinted_scaled_rotated_bitmap_region(enemy->image, offset, 0, 16, 16, al_map_rgb(tint_red, 255, 255), // (255, 255, 255) white
                0, 0, dx, dy, TILE_SIZE / 16, TILE_SIZE / 16,
                0, flag); 
        }
    }
    else if(enemy->status == DYING){
        /* [TODO Homework III 12.24] DYING ANIMATION */
        
        int dying_offset = 16 * (int)(enemy->death_animation_tick / 8);
        int flag = enemy->dir == RIGHT ? 1 : 0;            // Character flipping.

        if (enemy->type == slime) {
            al_draw_tinted_scaled_rotated_bitmap_region(enemy->image, dying_offset, 16, 16, 16, al_map_rgb(0, 255, 255),
                0, 0, dx, dy, TILE_SIZE / 16, TILE_SIZE / 16,
                0, flag);
        }

        if (dying_offset == 16 * 8) {
            enemy->status == DEAD;
        }
    }
...
void hitEnemy(Enemy * enemy, int damage, float angle){
    ...
    /* [TODO Homework I 12.24] DECREASE HEALTH WITH DAMAGE, SET STATUS TO `DYING` IF HEALTH < 0 */
    
    enemy->health -= damage;
    printf("[DEBUGGER] Enemy Damage: %d\n", damage);
    if (enemy->health <= 0){
        enemy->status = DYING;
    }
